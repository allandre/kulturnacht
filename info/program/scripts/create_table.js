...
function drawProgramTable() {
  ...
  $p.text(
    "Um mehr Informationen zu einem Anlass zu erhalten, auf den Eintrag klicken.",
  );
  $p.css("max-width", "unset");

  $containerDiv.append(
    '<div id="legend-program" class="legend">  \
                    <h6>Legende: </h6> \
                    <div>&#x1F46E; Führung</div> \
...
function processAdditionalRow($row, $cell, event, locationId) {
  ...
  if (!extraRows[event.participantId]) {
    ...
    if (/^@/.test(participant.description)) {
      ...
      if (!$extraRow) {
        console.log(
          "Das war genau der Fall, den du kanntest, aber zu faul warst, dich darum zu kümmern..",
        );
        return;
      } else {
      ...
      if (participant.images && participant.images.length > 0) {
        ...
        var $img = $("<img>", {
          src: "resources/participants/" + participant.images[0],
        });
        $imgDiv.append($img);
      }
...
function addClickEvent($cell, participantId, locationId) {
  $cell[0].setAttribute(
    "onclick",
    'toggleExtraRow(event, "' + participantId + '", "' + locationId + '");',
  );
}
