...
function updateProgramSection(force) {
  ...
  if (
    containerWidth > 747 &&
    (force ||
      programSectionState === programStates.undef ||
...
function adjustGallery() {
  if (isGalleryLoaded) {
    ...
    var oldNumberOfColumns = $participantGallery
      .children()
      .filter(".gallery-column").length;
    ...
    if (oldNumberOfColumns != newNumberOfColumns) {
      ...
      for (var i = 0; i < 2; i++) {
        ...
        for (var j = 0; j < $galleryColumns.length; j++) {
          ...
          var currentBottom =
            $currentLast.position().top + $currentLast.height();
...
function toggleParticipantInfo(evt, id) {
  ...
  if (
    ...
    evt.target.classList.contains("closebtn")
  ) {
...
function scrollToElement($element) {
  ...
  if (offset < 0) {
    ...
    $("html,body").animate(
      { scrollTop: $(window).scrollTop() + offset - 1.2 * $("nav").height() },
      1000,
    );
  }
}
...
function fixCloseGalleryButton() {
  if ($("#gallery-container").is(":visible")) {
    var distanceToTop =
      $("#gallery-container").offset().top - $(window).scrollTop();
    var bottomToTop =
      $("#gallery-container").offset().top +
